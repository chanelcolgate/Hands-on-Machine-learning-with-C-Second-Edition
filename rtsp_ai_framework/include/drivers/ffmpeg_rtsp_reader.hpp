...
public:
    ...
    bool open(const std::string& url) {
        ...
        // The VLC endpoint is confirmed to provide video over UDP.
        ...
        for (unsigned int i = 0; i < fmt_ctx_->nb_streams; ++i) {
            const AVCodecParameters* p = fmt_ctx_->streams[i]->codecpar;
            std::cout << "[FFmpegRtspReader] Stream " << i
                      << ": type=" << av_get_media_type_string(p->codec_type)
                      << ", codec=" << avcodec_get_name(p->codec_id);
        ...
        if (codec == nullptr) { close(); return false; }
        codec_ctx_ = avcodec_alloc_context3(codec);
        if (codec_ctx_ == nullptr) { close(); return false; }
        ret = avcodec_parameters_to_context(codec_ctx_, codecpar);
        if (ret < 0) { log_error("avcodec_parameters_to_context", ret); close(); return false; }
        ret = avcodec_open2(codec_ctx_, codec, nullptr);
        if (ret < 0) { log_error("avcodec_open2", ret); close(); return false; }

        frame_ = av_frame_alloc();
        frame_bgr_ = av_frame_alloc();
        if (frame_ == nullptr || frame_bgr_ == nullptr || codec_ctx_->width <= 0 || codec_ctx_->height <= 0) {
            close();
            return false;
        }

        int buffer_size = av_image_get_buffer_size(AV_PIX_FMT_BGR24,
            codec_ctx_->width, codec_ctx_->height, 1);
        buffer_ = static_cast<uint8_t*>(av_malloc(static_cast<size_t>(buffer_size)));
        if (buffer_ == nullptr) { close(); return false; }
        ...
        if (sws_ctx_ == nullptr) { close(); return false; }

        std::cout << "[FFmpegRtspReader] Connected using transport: udp\n"
                  << "[FFmpegRtspReader] Stream opened: " << width() << "x" << height() << std::endl;
        return true;
    }
    ...
    double get_fps() const {
        ...
        AVRational r = av_guess_frame_rate(fmt_ctx_, fmt_ctx_->streams[video_stream_idx_], nullptr);
        return r.num > 0 && r.den > 0 ? av_q2d(r) : 0.0;
    }
private:
    static void log_error(const char* op, int code) {
        char text[AV_ERROR_MAX_STRING_SIZE]{};
        av_strerror(code, text, sizeof(text));
        std::cerr << "[FFmpegRtspReader] " << op << " failed: " << text << std::endl;
    }
