...
int main() {
	...
	if (ret == true) {
		cout<<"Values are in increasing order"<<endl;
	} else {
		cout<<"Values are not in increasing order"<<endl;
	}

	int maxint = maxValue(v,10);
	cout<<"Max int value of array is "<< maxint <<endl;
	float fv[10];
	for (int i=0; i<10; i++) {
		fv[i] = i*10.5;
	}
	...
	cout<<"Max float value of array is "<<maxfloat<<endl;

	Coin c[10];
	bool res = inIncreasingOrder(c,10);
	if (res == true) {
		cout << "Coins are in increasing order" << endl;
	}
	Coin max = maxValue(c,10);
	cout<<"Max valued coin is "<<max<<endl;
}
