	...
	if (argc > 1) {
		...
		for (auto& dataset_name : dataset_names) {
			...
			if (fs::exists(dataset_full_name)) {
				...
				data::Load(dataset_full_name, dataset, info, true);
				...
				// std::cout << dataset.col(1) << std::endl;
				// std::cout << dataset.at(0, 1) << std::endl;
				// std::cout << dataset.at(1, 1) << std::endl;
				...
				std::cerr << "Dataset file " << dataset_name << " missed" << std::endl;
			}
		}
	}
	return 0;
