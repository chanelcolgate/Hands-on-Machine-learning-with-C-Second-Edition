...
int main() {
	...
	// Using 80% of data for training and remaining 20% for asserting MSE
	double validation_size = 0.2;
	HyperParameterTuner<LinearRegression<>, MSE, SimpleCV> parameters_tuner(
			validation_size,
			rand_samples,
			rand_labels
	);
	...
	plt.Draw2D(plotcpp::Points(x_coords.begin(), x_coords.end(),
			...
			plotcpp::Points(x_pred_coords.begin(), x_pred_coords.end(),
